...
def _format_function_calls(response: Any) -> str | None:
...
class TasksAgentHooks(AgentHooks[ChatContext]):
    async def on_start(
        self,
        context: RunContextWrapper[ChatContext],
        agent: Agent,
    ) -> None:
        # Start full agent timer
        context.context._agent_started_at = time.perf_counter()
    ...
    async def on_end(
        ...
        started_at = getattr(context.context, "_agent_started_at", None)
        duration_ms = None
    ...
    async def on_tool_start(
        ...
        context.context._tool_started_at = time.perf_counter()
    ...
    async def on_tool_end(
        ...
        started_at = getattr(context.context, "_tool_started_at", None)
        duration_ms = None
        if started_at is not None:
            duration_ms = round((time.perf_counter() - started_at) * 1000, 2)
        ...
        if isinstance(result, str):
            ...
            try:
                result_preview = json.dumps(result, indent=2, ensure_ascii=False, default=str)
            except Exception:
    ...
    async def on_llm_end(
        ...
        function_calls = _format_function_calls(response)
